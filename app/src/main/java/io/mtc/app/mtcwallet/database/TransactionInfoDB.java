...
public class TransactionInfoDB extends SQLiteOpenHelper {
    ...
    public List<TransactionInfo> getTransactionList(String address, String contract_address) {
        ...
        Cursor cursor = null;
        try {
            cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE (address_from = ? OR address_to = ?) AND contract_address = ? ORDER BY id DESC", new String[]{ address, address, contract_address });
